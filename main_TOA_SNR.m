...
%         Calculate distance from TOAs
        ...
        TOA_groundTruth = dist_groundTruth./init.soundSpeed*2;
        TOA_est = dist_est_from_method./init.soundSpeed*2;
    end
    RMSE_TOA(bb,v) = calculate_rmse(TOA_est,TOA_groundTruth);
    v=v+1;
end

if (init.plot_enable ==1) && (bb==init.iteration)
    plot(init.SNR',log10(RMSE_TOA),'-o')
    xlabel('SNR(dB)');
