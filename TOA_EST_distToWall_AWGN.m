...
candidateDelays=linspace(6,1000,1000);
...
for ll = 1:1:1
    k = 1;
for ii=1
        sourcePos=[ii,dimensions(2)/6,dimensions(3)/2];
        receivPos=[sourcePos(1:2),sourcePos(3)-distSourceToReceiv];
               ...
               while 1
                   for mm=rr:-1:1
                       ...
                       delayEst_value = delayEst'*delayEst_control_vec;
                        ...
                        [gainEst(mm,1),delayEst(mm,1),response(:,mm),costFunctionDelay2]=...
                         delayEstimationFIR_filter(signalReflectionsFft, signalCleanFft,response_sim,candidateDelays);
...
save(['RMSE_TOA_vs_Dist_SNR' num2str(zz) '_Fs_44_1k_simulation_50_MCRoom_10_10_6_AWGN.mat']);
end
