...
for zz=[0.1 0.5 1 1.5 2]
for ll = 1:1:50
    k = 1;
for ii=-40:5:40
        sourcePos=[zz,dimensions(2)/2,dimensions(3)/2];
        receivPos=[sourcePos(1:2),sourcePos(3)-distSourceToReceiv];
...
%         signalDirect=signalDirect(1:end);
        ...
        var_noise = var(fftfilt(RIRs,signalClean))/(10^(ii/10));
...
%         soundsc(signalReceived,Fs)
        ...
        %Introduce AWGN
%         signalReceived = awgn(signalReceived,ii, 'measured', 'dB');
%         soundsc(signalReceived,Fs)
...
%     plot(SNR',log10(RMSE_peak_picking), '-o')
    hold on
    plot(SNR',log10(RMSE_model_based_est),'-o')
    xlabel('SNR(dB)');
    ylabel('log_{10}(RMSE of TOA)(\tau)');
    legend('Distance to the wall = 0.1m', 'Distance to the wall = 0.5m', 'Distance to the wall = 1m', 'Distance to the wall = 1.5m', 'Distance to the wall = 2m');
%     title({'Performance of the two method at a','dist = 1 and reverbTime =0.8'})
end
save(['RMSE_TOA_vs_SNR_Dist_' num2str(zz) '_Fs_44_1k_simulation_50_MCRoom_10_10_6_Drone_Noise.mat']);
end
